...
Project relevance is not entirely model-owned. Exact dependency/protocol/provider matches and explicitly monitored ecosystem matches discovered from frozen Project Profile/source identity are authoritative inputs to the shallow stage. The provider receives compact reference IDs and may add only a short incremental note. Python resolves the final `relation + basis` per Change: ecosystem/module references cannot establish `direct`, an unrelated concrete dependency cannot be used as context merely because it exists in the profile, and a local projection mistake must not force regeneration of an otherwise valid whole Batch. The stable cached/product `ChangeInsight` remains human-readable; the compact provider wire schema is an implementation contract, not a product API.
